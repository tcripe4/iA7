...
// Modified to handle Edit Account as well
...
class CreateAccountDialog extends React.Component {
    ...
    constructor(props) {
        ...
        if (this.props.created) {
            ...
            const userData = JSON.parse(localStorage.getItem(this.props.userId));
            this.state = { origPassword: userData.password,
                        origAccountName: userData.accountName,
                        accountName: this.props.userId,
                        displayName: userData.displayName,
                        profilePicDataURL: userData.profilePicURL,
                        profilePicURL: userData.profilePicDataURL,
                        accountPassword: userData.password,
                        accountPasswordRepeat: userData.password,
                        accountSecurityQuestion: userData.securityQuestion,
                        accountSecurityAnswer: userData.securityAnswer};
        }
    }
    ...
    //landing page. 
    handleSubmit = (event) => {
        if (!this.props.created && this.state.accountName != this.state.origAccountName) {
            // Need to clear previous account data from localStorage and update userId
            localStorage.removeItem(this.state.origAccountName);
    ...
    <div className="modal" role="dialog">
        ...
        <div className="modal-content">
            ...
            <center>
            <h3 className="modal-title"><b>{this.props.created ? "Create New Account" : "Edit Account"}</b></h3>
            </center>
            ...
            <button role="submit" 
                ...
                <span className={this.props.create ? "fa fa-user-plus" : "fa fa-user"}></span>&nbsp;{this.props.create ? "Create Account" : "Update Account"}
            </button>
            </form>
            </div>
        </div>
